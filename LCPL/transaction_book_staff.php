...
<head>
    ...
    <link rel="stylesheet" href="assets/css/borrowbook_staff.css">
        <style>
        .table-wrapper {
            margin: 20px;
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        // Determine the CSS class for return_status
        $statusClass = isset($statusClasses[$row["return_status"]]) 
                       ? $statusClasses[$row["return_status"]] 
                       : 'default'; // Default class for unknown statuses

        echo "<tr>
                <td>" . $row["id"] . "</td>
                <td>" . htmlspecialchars($row["book_name"]) . "</td>  <!-- Display book name -->
        ...
        if (!empty($row["id_picture"])) {
            echo "<img src='/LCPL/id_pictures/" . $row["id_picture"] . "' alt='ID Picture' width='50' height='50' 
                onclick='openModal(\"/LCPL/id_pictures/" . $row["id_picture"] . "\")'>";
        } else {
            echo "N/A";
        }
        echo "</td>
                ...
                <td><span class='status " . $statusClass . "'>" . htmlspecialchars($row["return_status"]) . "</span></td>
              </tr>";
    }
...
?>
     ...
     <script>
        document.getElementById('filterBtn').addEventListener('click', () => {
            ...
            rows.forEach(row => {
                const cellValue = filterType === 'borrow_date' ? row.children[8].textContent : row.children[9].textContent;
                const rowDate = new Date(cellValue.trim());
